...
export function UploadClient() {
  ...
  const onFile = useCallback(async (file: File) => {
    ...
    try {
      if (!file.name.toLowerCase().endsWith(".csv")) {
        setError("Only .csv files are supported right now. Export your spreadsheet as CSV first.");
        return;
      }
  ...
  if (stage === "pick") {
    return (
      <div>
        <p className="text-[11px] uppercase tracking-[1.5px] text-slate">Step 1 of 3</p>
        <h2 className="mt-2 text-[20px] font-semibold text-ink">Pick a CSV file</h2>
        <p className="mt-2 text-[14px] text-ink/75">
          Up to 4 MB. Your spreadsheet should have a header row.
        </p>
        <div
          className="mt-5 cursor-pointer rounded-[16px] border-2 border-dashed border-whisper-gray bg-canvas-ice px-6 py-10 text-center transition-colors hover:border-outline-blue hover:bg-ghost-blue"
        ...
        >
          ...
          <p className="mt-1 text-[12px] text-slate">No data leaves StarUp.</p>
        </div>
        <input
          ref={fileInputRef}
          type="file"
  ...
  if (stage === "map" && preview) {
    return (
      <div>
        <p className="text-[11px] uppercase tracking-[1.5px] text-slate">Step 2 of 3</p>
        <h2 className="mt-2 text-[20px] font-semibold text-ink">Map columns</h2>
        <p className="mt-2 text-[14px] text-ink/75">
          <strong className="font-semibold">{filename}</strong> — {preview.rowCount} rows,{" "}
          {preview.columns.length} columns. StarUp guessed where each field lives; adjust below.
        </p>
        ...
        <h3 className="mt-8 text-[14px] font-semibold text-ink">Preview (first 5 rows)</h3>
  ...
  if (stage === "importing") {
    return (
      <div>
        <p className="text-[11px] uppercase tracking-[1.5px] text-slate">Step 3 of 3</p>
        <h2 className="mt-2 text-[20px] font-semibold text-ink">Writing rows…</h2>
        <p className="mt-2 text-[14px] text-ink/75">
          This takes about a second per 100 rows.
        </p>
      </div>
    );
  }
  ...
  if (stage === "done" && result) {
    return (
      <div>
        <p className="text-[11px] uppercase tracking-[1.5px] text-slate">Done</p>
